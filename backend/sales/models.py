...
class Sale(models.Model):
    customer = models.ForeignKey('parties.Customer', on_delete=models.PROTECT, related_name="sales")
    invoice_number = models.CharField(max_length=100, unique=True)
    sale_date = models.DateField()
    total_amount = models.DecimalField(max_digits=18, decimal_places=2, default=0.00)
    total_paid = models.DecimalField(max_digits=18, decimal_places=2, default=0.00)
    total_debt = models.DecimalField(max_digits=18, decimal_places=2, default=0.00)
    settlement_status = models.CharField(max_length=50, default="PENDING")
    notes = models.TextField(blank=True, null=True)
    created_by_user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="created_sales")
